...
import { addDictTypeApi, type DictApi } from '#/api';
...
const formState = ref<Partial<DictApi.TypeAddModel>>(cloneDeep(defaultState));

const { loading, runAsync } = useRequest(addDictTypeApi, requestConf);
...
function onConfirm() {
  formInstance.value?.validate(async (valid) => {
    if (valid) {
      const state = cloneDeep(formState.value as DictApi.TypeAddModel);
...
<template>
  <Modal
    ...
    :title="$t('zen.service.dict.create')"
